...
st.markdown('<p style="{}"> 👀 AI Data Science Instructor </p>'.format(";".join([f"{k}:{v}" for k, v in title_style.items()])), unsafe_allow_html=True)
...
model = gemini.GenerativeModel(model_name="gemini-1.5-pro-latest",
                               system_instruction="""You are a user friendly AI Assistant to resolve data science
                               Queries of the user.""")

if "messages" not in st.session_state.keys():
    st.session_state.messages = [
        {"role": "assistant", "content": "Hello, this is Gemini and how can I help you?"}
    ]
    st.title("📢:rainbow[Welcome user, How may I help you today?]")
